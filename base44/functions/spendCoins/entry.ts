 ...
 *           { amount: 3, refund: true, reason: '..._refund' } — credits coins
 *           back (used when the client pays first and the AI call then fails).
...
export default async function(req) {
  try {
    ...
    // Refund: credit coins back. Used when the client pays FIRST and the AI
    // call then fails — the user never pays for an undelivered answer.
    if (refund) {
      ...
      console.log(`[Base44] Coins refunded: +${amount} (${body.reason || 'unknown'}) | User ${user.email} | Remaining: ${remaining}`);
      return Response.json({ success: true, refunded: amount, remaining });
    }
