...
export default function DocumentReviewModal({ doc, analysis, loans, bills, onClose, onDone }) {
  ...
  const [keepMisc, setKeepMisc] = useState(null); // null = not asked yet
  ...
  const raymaMessage = analysis?.rayma_message || doc.notes || "I've analyzed this document. Please review the extracted details below.";

  async function handleApprove() {
    setSaving(true);
    // Log to appropriate entity
        ...
        {step === "misc_prompt" ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="ghost" className="rounded-xl text-xs text-destructive hover:text-destructive" onClick={() => handleKeepMisc(false)} disabled={saving}>
                <X className="w-3.5 h-3.5 mr-1" /> No, Discard
              </Button>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm font-semibold text-foreground mt-0.5">{doc.document_type || "Unknown"}</p>
            </div>
            ...
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground uppercase tracking-wider">Extracted Data — Please Verify</Label>
              {fields.amount != null && (
                <div>
              ...
              {fields.payee && (
                <div>
                  <Label className="text-[10px] text-muted-foreground">Payee / From</Label>
              ...
              {Object.keys(fields).length === 0 && (
                <p className="text-xs text-muted-foreground italic">No structured data extracted. You can still save this document.</p>
              )}
            </div>

            <div className="flex gap-2 pt-2">
              <Button onClick={handleApprove} disabled={saving} className="flex-1 rounded-xl text-xs h-9">
                <Check className="w-3.5 h-3.5 mr-1" /> {folder === "payments" || folder === "bills" ? "Approve & Log" : "Approve & Save"}
              </Button>
