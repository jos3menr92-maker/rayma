    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{T("latePaymentLog", "Late Payment Log")}</span>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="px-4 pb-4 border-t border-border pt-3 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{T("amountLabel", "Amount")} ($)</label>
              <input
                type="number"
                step="any"
                required
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{T("dateLabel", "Date")}</label>
              <input
                type="date"
                value={form.date}
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{T("note", "Note")}</label>
              <input
                type="text"
                value={form.reason}
                onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                placeholder={T("missedEx", "e.g. Missed March")}
                className="w-full bg-muted border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </div>
          </div>
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="flex-1 bg-amber-500 text-white py-2 rounded-xl text-sm font-semibold">
              {saving ? T("saving", "Saving...") : T("logEntry", "Log Entry")}
            </button>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {adj.date && new Date(adj.date + 'T00:00:00').toLocaleDateString(locale, { month: 'short', day: 'numeric', year: 'numeric' })}
      ...
      {adjustments.length === 0 && !showForm && (
        <p className="text-xs text-muted-foreground text-center py-3 border-t border-border">
          {T("tapToLogLate", "Tap + to log a late or missed payment")}
        </p>
      )}
    </div>
  );
