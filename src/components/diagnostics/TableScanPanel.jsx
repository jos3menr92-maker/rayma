...
import { Database, Loader2, CheckCircle2, XCircle, FlaskConical, RefreshCw, Lock } from "lucide-react";
          ...
          {entries.map(([table, result]) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs ${isError ? "text-destructive" : "text-primary"}`}>{result}</span>
              </div>
            );
          })}
        </div>
    ...
    <div className="bg-card rounded-xl border border-border p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold font-heading text-foreground flex items-center gap-2">
          <Database className="w-5 h-5 text-primary" /> {T("scanAndRepair", "Scan & Repair")}
        </h3>
        <button
          onClick={onLock}
          className="text-xs text-muted-foreground hover:text-destructive flex items-center gap-1 transition-colors"
        >
          <Lock className="w-3.5 h-3.5" /> {T("lock", "Lock")}
        </button>
      </div>
      ...
      <div className="mb-5">
        ...
        <Button onClick={runScan} disabled={scanning} variant="secondary" size="default" className="w-full">
      ...
      {scanResults && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-5">
          ...
          <Button onClick={runSeed} disabled={seeding} size="default" className="w-full">
            {seeding ? <Loader2 className="w-4 h-4 animate-spin mr-1.5" /> : <FlaskConical className="w-4 h-4 mr-1.5" />}
      ...
      {seedResults && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Button onClick={runConfirm} disabled={confirming} size="default" className="w-full">
