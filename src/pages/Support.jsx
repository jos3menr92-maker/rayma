...
export default function Support() {
  ...
  async function handlePurchase(planId) {
    ...
    if (isNativeApp) {
      if (window.ReactNativeWebView) {
        ...
        setError(T("nativeAppError", "Could not connect to the native app store. Please restart the app."));
        setLoading(null);
      }
      return;
    }
  ...
  async function handlePromoCode(e) {
    ...
    if (res.data?.success) {
      ...
      setError(res.data?.error || T("invalidPromoCode", "Invalid promo code."));
    }
  }
    ...
    <div className="max-w-xl mx-auto px-4 pt-6 pb-24">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold font-heading text-foreground mb-2">{T("raymaStation", "RAYMA Power Station")}</h1>
          <p className="text-sm text-muted-foreground leading-relaxed">
            {T("batteryDesc", "Every user gets an AA Battery (10 Energy Bars) every day for free. Need more juice to simulate heavy debt payoffs? Upgrade your capacity below.")}
          </p>
        </div>
        ...
        {(successType || promoSuccess) && (
          <div className="mb-6 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-green-700 dark:text-green-400">{T("batteryRecharged", "Your AI Battery has been recharged and upgraded.")}</p>
            </div>
          </div>
        )}
        ...
        <div className="stripe-checkout-ui">
          ...
          <div className="mb-6 bg-primary/5 border border-primary/30 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="font-semibold text-foreground text-sm">{T("cheatCode", "Have a Cheat Code? (Promo)")}</p>
            </div>
          ...
          <p className="text-center text-xs text-muted-foreground mt-6 leading-relaxed">
            {T("paymentsSecure", "Payments are securely processed. Purchases are tied to your RAYMA account.")} <br />
            {T("priceVary", "Prices may vary automatically based on your local currency to ensure fair access globally.")} <br />
            {T("seeTerms", "See")} <a href="/terms" className="underline text-primary">{T("termsOfService", "Terms of Service")}</a> {T("fullDetails", "for full details")}.
          </p>
        </div>
      </motion.div>
    </div>
